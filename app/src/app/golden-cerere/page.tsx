...
import { runRequestGolden } from "@/lib/request-golden";
import { Icon } from "@/components/ui/Icon";

/**
 * Golden set vizibil pentru „Strigarea" (Etapa 4) — rulează frazele de cerere
 * prin parserul determinist și arată rata de reușită (inclusiv „vag → întrebare").
 * Plasa de regresie când legăm Gemma în spatele aceluiași `ModelProvider`.
 */
export default function GoldenCererePage() {
  const { passed, total, results } = runRequestGolden();
          ...
          <div className="h-[60px] px-md flex items-center justify-between">
            <div className="flex items-center gap-base min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="text-[16px] font-semibold text-on-surface leading-tight truncate">Golden set · Strigarea</h1>
                <p className="text-[11px] text-on-surface-variant leading-tight truncate">regresie structurare cerere</p>
              </div>
            </div>
        ...
        <main className="flex-1 pt-[60px] pb-lg px-md">
          <div className={`mt-md rounded-card p-md ${ok ? "bg-chip-bg" : "bg-error-container"}`}>
            ...
            <p className="text-[13px] text-on-surface-variant mt-1">
              {pct}% structurate corect {ok ? "· peste pragul de 90%" : "· sub pragul de 90%"}
            </p>
          </div>
