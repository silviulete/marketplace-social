...
import { runLedger } from "@/lib/ledger-scenarios";
import { Icon } from "@/components/ui/Icon";

/**
 * Regresie vizibilă a plății + escrow + ledger (Etapa 7) — plată→escrow,
 * eliberare→producători+fond, refund→cumpărător, split multi-producător,
 * rotunjire la fond și invariantul de reconciliere (suma ledger = 0).
 */
export default function BaniPage() {
  const scenarios = runLedger();
        ...
        <header className="fixed top-0 z-40 w-full max-w-phone bg-background/85 backdrop-blur-md border-b border-card-border">
          <div className="h-[60px] px-md flex items-center justify-between">
            <div className="flex items-center gap-base min-w-0">
              <Icon name="account_balance" className="text-primary" size={24} filled />
              <div className="min-w-0">
                <h1 className="text-[16px] font-semibold text-on-surface leading-tight truncate">Bani · escrow + ledger</h1>
                <p className="text-[11px] text-on-surface-variant leading-tight truncate">regresie plată + reconciliere (suma = 0)</p>
              </div>
            </div>
        ...
        <main className="flex-1 pt-[60px] pb-lg px-md">
          <div className={`mt-md rounded-card p-md ${ok ? "bg-chip-bg" : "bg-error-container"}`}>
            ...
            <p className="text-[13px] text-on-surface-variant mt-1">
              asserturi trecute {ok ? "· ledger-ul se reconciliază (dublă partidă)" : "· verifică eșecurile"}
            </p>
          </div>
