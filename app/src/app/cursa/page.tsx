...
import { runCursa } from "@/lib/cursa-scenarios";
import { Icon } from "@/components/ui/Icon";

/**
 * Regresie vizibilă a mașinii de stări a Cursei (Etapa 6a) — rulează scenariile
 * prin ceasul simulat (rutare la cutoff, accept→livrare, refuz→refund, expirare,
 * gărzi). Logică pură, zero UI de produs; aici doar verdictul testelor.
 */
export default function CursaPage() {
  const scenarios = runCursa();
        ...
        <header className="fixed top-0 z-40 w-full max-w-phone bg-background/85 backdrop-blur-md border-b border-card-border">
          <div className="h-[60px] px-md flex items-center justify-between">
            <div className="flex items-center gap-base min-w-0">
              <Icon name="schedule" className="text-primary" size={24} filled />
              <div className="min-w-0">
                <h1 className="text-[16px] font-semibold text-on-surface leading-tight truncate">Cursa · mașina de stări</h1>
                <p className="text-[11px] text-on-surface-variant leading-tight truncate">regresie cutoff + accept/refuz (Clock-driven)</p>
              </div>
            </div>
        ...
        <main className="flex-1 pt-[60px] pb-lg px-md">
          <div className={`mt-md rounded-card p-md ${ok ? "bg-chip-bg" : "bg-error-container"}`}>
            ...
            <p className="text-[13px] text-on-surface-variant mt-1">
              asserturi trecute {ok ? "· mașina de stări se comportă conform planului" : "· verifică eșecurile"}
            </p>
          </div>
